...
$refresh=$_REQUEST["refresh"];
    ...
    if (mysqli_num_rows($result) > 0) {
        while ($res = mysqli_fetch_array($result)) {
            ...
            $one_line_array[$j][16]=$res['algo_desiderative'];
            $one_line_array[$j][17]=$res['comments_desiderative'];
           	$j++;
...
?>
<a href="/compare/compare_desiderative.php?refresh=1">Обновить данные</a>
<?
...
$command="VR-DS-PrS-Pr";

$algo_db="algo_desiderative";
$comments_db="comments_desiderative";
$lico=3;
$chislo=1;
...
for($i=0;$i<count($one_line_array);$i++)
{
    for($j=0;$j<count($array_alg);$j++)
    {
        ...
        if($one_line_array[$i][0]==$array_alg[$j][0]&&$one_line_array[$i][1]==$array_alg[$j][1])
        {
            ...
            $kochergina=$one_line_array[$i][10];
            if($kochergina=="0")
            {
            ...
            $url="/generator2.php?id=$verb_id&command=$command&lico=$lico&chislo=$chislo&pada=$verb_pada";
            ...
            elseif($verb_pada=="U")
            {
                ...
                else
                {
                    ...
                    echo '<tr '.$class.'><td>'.$one_line_array[$i][1]." ".$omonim.'</td><td>'.$one_line_array[$i][2].'</td><td>'.$one_line_array[$i][4].'</td><td>'.$one_line_array[$i][3].'</td><td>'.$verb_pada.'</td><td>'.$verb_ryad.'</td>           <td>'.$verb_type.'</td><td>'.$kochergina.'</td><td><a href="'.$url.'" class="link-secondary link-offset-2 link-underline-opacity-25 link-underline-opacity-100-hover">'.$string.'</a></td>
                    <td>'.$compare.'</td></tr>';
                

                $query = "UPDATE kochergina SET $algo_db=?,$comments_db=? WHERE id=?";
            ...
            else
            {
                echo '<tr><td>'.$one_line_array[$i][1]." ".$omonim.'</td><td>'.$one_line_array[$i][2].'</td><td>'.$one_line_array[$i][4].'</td><td>'.$one_line_array[$i][3].'</td><td>'.$verb_pada.'</td><td>'.$verb_ryad.'</td>           
                        <td>'.$verb_type.'</td><td>'.$kochergina.'</td><td><a href="'.$url.'">'.$one_line_array[$i][16].'</a></td><td>'.$one_line_array[$i][17].'</td></tr>';
            }
