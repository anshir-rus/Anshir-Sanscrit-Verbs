...
echo '<thead><tr><th scope="col">Корень</th>

<th scope="col" style="background: #20c997;">Удвоенный корень из примера</th><th scope="col" style="background: #ffc107;">Генерация алгоритмом</th>
<th scope="col">Совпадение</th></tr></thead>';
...
for($i=0;$i<count($mool_array);$i++)
{
    ...
    //print_r($desiderative);
    
    //$desiderative=simple_sandhi($desiderative,"","DS",1)[0];
    $desiderative=str_replace("|","",$desiderative);
    
    if($desiderative==$answer_double_array[$i])
    {
        $compare="Да";
        $class='';
    }
    else
    {
        $array=explode(",",$desiderative);
            ...
            if($array[0]==$answer_double_array[$i]||$array[1]==$answer_double_array[$i])
            {
    ...
    <td>".$answer_double_array[$i]."</td><td>$desiderative</td><td>$compare</td>";	
    echo "</tr>";
}
